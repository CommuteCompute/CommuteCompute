...
### 23.13 No Hardcoded Timing Literals in Engine Route Templates (v5.9.0)

**[CRITICAL]:** The CommuteCompute™ engine MUST NOT embed numeric timing literals in any route template, fallback, or leg builder. Every leg duration MUST be derived from real per-user data — there is no valid reason for a literal like `totalMinutes: 35` or `minutes: 6` to appear adjacent to a tram, train, or bus leg definition.

**Background:** Prior to v5.9.0, `src/engines/commute-compute.js::getHardcodedRoutes()` contained 28 timing literals embedded in 6 route templates (`totalMinutes: 35`, `tram minutes: 6`, `train minutes: 8`, …). These literals assumed a Melbourne-CBD commute pattern and produced incorrect totals for any user with a unique commute. The founder directive closing this gap (2026-04-11) mandated removal.

**Required replacement — `src/engines/dynamic-routes.js`:**
- **Walk legs**: haversine distance between lat/lon coordinates, divided by a configured walking speed (default `WALKING_SPEED_M_PER_MIN = 80` m/min, Victorian Department of Transport planning default). `minutes = ceil(distance_m / speed_m_per_min)`.
- **Transit legs**: GTFS static `stop_times.txt` lookup when available (optional injected helper); otherwise geodesic fallback using per-mode Victorian PTV service-standard averages: `tram = 15 km/h`, `train = 45 km/h`, `bus = 20 km/h`. These constants are mode characteristics, NOT location-specific, and are the ONLY timing constants permitted outside the walking-speed default.
- **Coffee legs**: `COFFEE_DURATION_MINUTES = 5` (user experience duration, overridable via `locations.cafe.dwellMinutes`). Not a commute literal.
- **Aggregation**: raw (unrounded) minutes are summed across legs, and `Math.ceil` is applied ONCE at the route-total boundary. Per-leg `Math.round` is prohibited in dynamic-routes.js because it compounds rounding error across the journey (v5.9.0 T14, closes N17).

**Allowlist — acceptable universal constants:**

| Constant | Value | Source | Why it's permitted |
|---|---|---|---|
| `WALKING_SPEED_M_PER_MIN` | 80 (1.33 m/s) | Victorian DoT planning default | Physical property of the user, not the place |
| `MODE_AVERAGE_SPEEDS_KMH.tram` | 15 | Victorian PTV service standard | Mode characteristic, not route-specific |
| `MODE_AVERAGE_SPEEDS_KMH.train` | 45 | Victorian PTV service standard | Mode characteristic, not route-specific |
| `MODE_AVERAGE_SPEEDS_KMH.bus` | 20 | Victorian PTV service standard | Mode characteristic, not route-specific |
| `COFFEE_DURATION_MINUTES` | 5 | User experience default | Not commute-related; overridable per user |

**Prohibited (rejected in review):**
- Any `totalMinutes: N` literal in route template definitions.
- Any `minutes: N` literal inside a leg definition where N represents a travel time.
- Any hardcoded route patterns that assume a specific line, terminus, or CBD destination.
- Suburb-derived fallback stop names (already prohibited by N2-guard in v5.8.2; reinforced here).

**Enforcement:** The comprehensive compliance audit greps for `totalMinutes:\s*\d+` and `minutes:\s*\d+` patterns in `src/engines/*.js` and `api/commutecompute.js` and fails on any match outside the allowlist file (`src/engines/dynamic-routes.js`). Add new template shapes to `ROUTE_TEMPLATE_SHAPES` in that file — never add new literals.

### 23.14 Journey Legs Always Populate — Non-Commute Days Are a Renderer Overlay (v5.9.1)

**[CRITICAL]:** `/api/commutecompute?format=json` MUST return a non-empty `journey_legs` array AND a non-empty `alternatives` array on every successful request, regardless of `isCommuteDay`, `isTomorrowCommute`, or `summary.status`. The morning commute is always computed; non-commute days ("NO COMMUTE TODAY", weekends, public holidays) are a RENDERER overlay drawn on top of a populated leg list — NOT a data-suppression signal that empties the engine output.

**Background:** v5.9.0's T1 refactor silently broke this invariant when `src/engines/dynamic-routes.js::resolveLocationToken` could not read `lat`/`lon` fields off `findNearestStops` results (the helper had never included them, and the new dynamic engine was the first caller to require them). Every route template aborted, `discoverRoutes` returned `[]`, and the rendered dashboard displayed a blank journey area between the header and the "WORK — ..." footer. The 4-agent verification swarm missed the regression because no agent visually inspected the rendered dashboard preview; they all treated `journey_legs: []` as expected Saturday behaviour. It is not.

**Required verification rules (v5.9.1 U5):**
1. `/cc-deploy-verify` Agent A MUST assert `journey_legs.length > 0` AND `alternatives.length > 0` on the live `/api/commutecompute?format=json` response as a BLOCKING check before any other verification, regardless of day or status. Failure is a blocking regression.
2. `/cc-deploy-verify` Agent D (blind investigator) MUST use the Chrome browser automation tools to navigate to the admin panel, take a screenshot, and visually confirm the rendered TRMNL display preview contains visible leg rows (walk / coffee / tram / train / bus). A blank journey area between the header and the footer is a BLOCKING regression.
3. Any future refactor of the engine's route-generation path MUST be smoke-tested end-to-end against a real coordinate pair (not just source-level code inspection) before being considered complete.

**Rationale:** "No commute today" is a USER message, not an ENGINE state. The engine always has enough information to compute the morning commute from the configured home → work pair; suppressing leg output on non-commute days is a category error because the admin panel and device preview use the same leg list to display "this is what your tomorrow commute would look like" in tomorrow mode. Empty legs break tomorrow mode, terminus-as-work configurations, and the entire confidence calculation. The invariant is: **compute always, render with day-specific framing**.

### 23.15 Tram Stop-Match Pipeline — Identity First, Proximity Last (v5.9.2)

**[CRITICAL]:** The tram GTFS-RT stop-match cascade MUST run identity-based tiers before fuzzy proximity matching. Coordinate proximity (300 m) is a LAST-RESORT fallback only — never a primary identity mechanism.

**Background:** Prior to v5.9.2, the tram cascade in `src/services/opendata-client.js::getDepartures` called `processCoordinateProximitySearch` (Tier 2) before `processAllTripsStopSearch` and short-circuited when coord-proximity returned three or more results. Coord-proximity uses a 300 m radius around the configured tram stop and accepts any tram trip passing within that distance — including trips on parallel streets. For the locked tram stop on the configured boarding street, this produced matches from a parallel-street route (~236 m away) instead of the correct locked route on the configured street. Live cross-referencing against the operator's real-time departure board repeatedly surfaced the wrong route across v5.8.1, v5.8.2, v5.9.0 and v5.9.1.

The fix in v5.9.2 is an architectural reordering of the cascade plus a new identity-resolution tier that bridges the feed/static stop-ID namespace gap using the same static coordinate lookup (`VIC_TRAM_STOPS_WITH_COORDS`) that was already present in the repo. **No hardcoded data, no committed snapshot, no build scripts** — the bridge is pure runtime computation from the live feed plus existing coordinate data.

**Required cascade order for tram stop-match** (`src/services/opendata-client.js::getDepartures` for `mode === 'tram'`):
1. **`processGtfsRtDepartures`** — exact stop-id match via `matchesStopId` (handles prefixed `aus:vic:tram-NNNN` via suffix-colon/suffix-dash). Source: `gtfs-rt`.
2. **Alt stop ID retry** — if the engine pre-computed alternative feed-side IDs via `altStopIds`, retry `processGtfsRtDepartures` for each. Source: `gtfs-rt`.
3. **`processAllTripsStopSearch`** — lenient trailing-numeric matching (last 3–4 digits). Source: `gtfs-rt-scan`.
4. **`processCoordIdentityMatch`** *(v5.9.2 NEW, revised in v5.9.3)* — bridges the feed/static ID namespace gap by comparing the STATIC coordinates of each feed stop ID against the target stop's coordinates with a named constant radius `TRAM_COORD_IDENTITY_RADIUS_METRES` (currently 40 m). A match within the radius means "same physical platform, different ID format in the feed". Source: `gtfs-rt-coord-identity`.
5. **`processRouteLevelDepartures`** — fallback when stop-level identity fails and the caller has provided a known route or line code. Median-stop heuristic with ~10–20 minute timing error, so it is explicitly NOT treated as stop-level identity evidence. Source: `gtfs-rt-route`.
6. **`processCoordinateProximitySearch`** — LAST RESORT only. 300 m fuzzy radius. Fires only when tiers 1–5 all return zero. Source: `gtfs-rt-coord`.

**The coord-identity threshold is deterministic**, not heuristic. Revised in v5.9.3 from 15 m to 40 m based on direct haversine measurements against the static `VIC_TRAM_STOPS_WITH_COORDS` dataset:
- Tram platforms are typically under 10 m wide.
- Inbound/outbound directional pairs of the same physical tram stop sit **within ~25 m** of each other across the Melbourne tram network — wide-road crossings can produce pair distances up to ~25 m. The v5.9.2 value of 15 m was too tight and left the identity tier inert whenever the feed used the outbound platform ID at a wide crossing.
- Same-route adjacent stops on the same street are always **more than 150 m** apart.
- Adjacent stops on parallel streets (the closest false-match class) are always **more than 200 m** away (measured minimum 258 m on the tightest pair in the dataset).
- 40 m therefore guarantees that a coord-identity match IS the target physical platform, not a neighbour (25 m paired-platform maximum + 15 m safety margin; 110 m+ margin against the nearest false-match class).
- The threshold is a universal physical constant of the Victorian tram network and MUST be a named module-level constant (`TRAM_COORD_IDENTITY_RADIUS_METRES`) per §13.3, with the measured rationale in its JSDoc. No per-user, per-stop, or per-route configuration.

**Prohibited patterns (review reject):**
- Adding any new tier above coord-identity that uses fuzzy geographic matching wider than `TRAM_COORD_IDENTITY_RADIUS_METRES`. The only legitimate tier between stop-level exact and coord-identity is lenient trailing-numeric scan.
- Hardcoded feed-stop-id → static-stop-id mappings. The bridge MUST be via runtime haversine against `VIC_TRAM_STOPS_WITH_COORDS` — no snapshot files, no build scripts that produce mapping data, no committed JSONs.
- Short-circuiting the cascade when coord-identity returns a non-empty result if earlier tiers might also have results. All identity-based tiers (T1–T5) merge their results via trip-id dedup before falling to T6.
- Calling `processCoordinateProximitySearch` (T6) when T4 (`processCoordIdentityMatch`) has any output. T6 is last-resort only.
- Classifying `gtfs-rt-route` as a stop-level identity source. Route-level is a median-stop heuristic, not an identity match, and must not be added to any `STOP_LEVEL_SOURCES` set.
- **Pre-filtering the coord-identity tier by a route number (v5.9.4 Z1 — MANDATORY):** `processCoordIdentityMatch` MUST NOT `continue`-skip feed trip entities whose `routeId` does not match a caller-supplied `targetRouteNumber`. The whole purpose of the identity tier is to DISCOVER which route is physically at the user's stop from the live feed. Filtering by a pre-guessed route defeats the tier completely — whenever the upstream route guess is wrong (e.g. due to a previously-poisoned KV value or a stale engine plan), the tier returns zero matches and the cascade falls through to lower-confidence tiers, perpetuating the wrong-route state. If a caller supplies `targetRouteNumber`, it MUST be used ONLY as a nearest-distance TIE-BREAK preference, never as an exclusion. This rule was added after v5.9.3 live verification showed the coord-identity tier returning `feedStopIdsResolved: []` in the presence of successful stop-level matches — the tier had been filtered out by a wrong upstream route preference.

**KV write-once + stop-level gate (v5.9.3 Y3 + v5.9.4 Z2 — MANDATORY):** Frequency-detection and feed-majority KV writes in the engine's tram route selection cascade (`api/commutecompute.js::commuteCompute`) MUST be gated on BOTH of these conditions simultaneously:
1. `preferredTramRoute` is currently empty (the write-once constraint), AND
2. The selected route is backed by at least one stop-level-confident tram entry — `bestIsStopLevel === true` in the frequency-detect branch, or `selectedIsStopLevel === true` (derived from `STOP_LEVEL_SOURCES.has(t.source)`) in the feed-majority fallback branch.

Coord-tier (`gtfs-rt-coord`) and route-level (`gtfs-rt-route`) cascade wins MAY drive the current request's `selectedRoute` for rendering, but MUST NOT persist to `cc:preferred_tram_route`. An empty-KV write of a non-stop-level result is classified as **cold-start poisoning** and is a BLOCKING regression.

This strengthened rule was added in v5.9.4 after the v5.9.3 live deploy exhibited cold-start KV poisoning: the first request immediately after deploy found the tram feed with no stop-level match for the user's stop, the cascade fell through to the route-level heuristic tier, frequency detection selected a non-stop-level route, and the naive v5.9.3 write-once guard (checking only `!preferredTramRoute`) persisted the wrong value to the empty KV. Subsequent requests then read back the poisoned value and the system oscillated between the correct route (when stop-level succeeded) and the poisoned route (when it fell to coord-tier).

The stored KV value legitimately changes only via:
- The v5.8.2 C4-clear block in `api/admin/preferences.js` on home/work address change.
- Explicit admin override through `setStationOverrides`.
- A factory reset / setup wizard flow.
- A one-off v5.9.4 Z3 migration wipe (consumed after first post-deploy request).

Any future introduction of an unconditional or under-gated `setPreferredTramRoute` call in the cascade is a regression. **Test coverage**: `tests/test-tram-cascade-selection.js` exercises the gate across eight scenarios including cold-start with stop-level, cold-start with route-level only, write-once protection, admin override precedence, and stop-level-vs-route-level frequency-detection preference. Any change to the inline cascade in `api/commutecompute.js` MUST update the pure-function replica `selectTramRoute` in the test file in lock-step and re-run the suite.

**Required diagnostic fields in `_liveDataDiag`** (exposed by `api/commutecompute.js`):
- `tramRouteSelectionConfidence`: one of `exact-stop-id`, `lenient-scan`, `coord-identity`, `route-level`, `coord-proximity-only`. Derived from the `source` field of the live trams array — any other value is a regression.
- `tramMatchTier`: the `matchMethod` returned by `getDepartures._feedInfo`, indicating which cascade tier actually matched.
- `tramFeedStopIdsResolved`: array of feed stop IDs that resolved to static coordinates within `TRAM_COORD_IDENTITY_RADIUS_METRES` of the target. Empirical observation (not hardcoded mapping), useful for cross-reference against TramTracker / Transport Victoria.
- `tramIdentityRadiusMetresUsed`: the radius actually applied for the current request (read from the `coord-identity` cascade attempt). Should equal `TRAM_COORD_IDENTITY_RADIUS_METRES` unless the caller overrides.
- `tramCascadeAttempts` *(v5.9.3 NEW)*: the full cascade attempt array from `trams._feedInfo.cascadeAttempts` — one entry per tier attempted with `{ tier, tried, found, ...tier-specific-extras }`. Lets the admin panel and verification agents surface which tier resolved the selected route for the current cycle.
- `tramCoordIdentityDebug` *(v5.9.4 NEW)*: diagnostic capture from the coord-identity tier — `{ totalUniqueFeedStopIds, matchedCount, radiusMetres, sampleLookups: [{ feedStopId, resolvedExact, resolvedViaNumeric, lookupLat, lookupLon, distanceToTargetM }, ...] }`. Lets any future coord-identity zero-match be diagnosed in a single deploy cycle — inspect `sampleLookups` to see whether the lookups are failing (all `distanceToTargetM === null`), succeeding outside the radius (all `> radiusMetres`), or succeeding inside the radius but the tier still returned empty (which would indicate a logic bug in `processCoordIdentityMatch`).

**Verification requirement (`/cc-deploy-verify` Agent A):** It is a BLOCKING regression if `tramRouteSelectionConfidence === 'coord-proximity-only'` while `tramFeedEntities >= 3`. That combination means the pipeline had plenty of data but was unable to resolve any identity-based tier — indicating either the cascade has been reordered incorrectly or the identity radius has been compromised.

**GTFS-RT feed sparsity reality (v5.9.5 AA4 — MANDATORY BACKGROUND):** GTFS-RT tram feeds truncate `stopTimeUpdate` to a forward window of upcoming stops. In any given 30-second cache window, a tram approaching or past the user's target stop may not have the target stop in its `stu` sequence at all. All four identity tiers (T1 exact, T2 alt-stop-id, T3 lenient scan, T4 coord-identity) will correctly return 0 matches in those windows — NOT as a regression but as the honest representation of feed state. The cascade then falls through to T5 route-level with `preferredTramRoute` from KV as the filter, which is SAFE because the v5.9.4 Z2 gate ensured KV was only populated during a cycle when stop-level was confident. The displayed route identifier remains correct because route-level filters by the KV-preserved route; timing accuracy drops to ~10-20 min per the route-level median-stop heuristic, but the route is right.

Live verification (v5.9.4 8-fetch run across an arbitrary VIC tram stop) confirmed that more than half of consecutive cache windows can have all stop-identity tiers return 0 — this is inherent GTFS-RT producer behaviour, not a CC bug. Any verification rule that demands always-identity-tier confidence across cache windows will misfire on every deployment.

**Coord-identity cross-tier divergence assertion (v5.9.5 AA3/AA5 — MANDATORY, replaces the v5.9.4 coord-identity health assertion):** The coord-identity tier (T4) exposes a `_liveDataDiag.tramCoordIdentityDebug.divergenceReport` field when T1 or T2 found matches. For each trip T1/T2 matched, the report records whether T4 also matched it and, if not, the rejection reason:

- **`matched`** — T4 also found the trip (no divergence) ✓
- **`no-dep-time`** — the nearest-within-radius stu on the trip had no `departure.time` or `arrival.time`. Legitimate stu-level edge case; PASS WITH NOTE.
- **`past-time`** — the nearest stu's time was in the past. Legitimate feed-timing artefact; PASS WITH NOTE.
- **`beyond-window`** — the nearest stu's time was > 120 min away. Legitimate forward-window edge case; PASS WITH NOTE.
- **`lookup-failed`** — `lookupTramStop(stu.stopId)` returned null for every stu in the trip. BLOCKING regression — indicates namespace bridging has broken (the feed uses an ID format `lookupTramStop` can't resolve).
- **`out-of-radius`** — all of the trip's stus that resolved via `lookupTramStop` were more than `TRAM_COORD_IDENTITY_RADIUS_METRES` from the target. BLOCKING regression — indicates the radius constant has been compromised or the feed's static coordinates have drifted, because T1/T2 found the trip via matchesStopId yet no stu in the trip resolves near the target.
- **`not-seen`** — the trip ID was in the known-matched set but T4 never iterated a trip with that ID. Should be impossible on a healthy feed; indicates a trip-id mismatch between T1/T2 and T4 iteration, likely a feed-decoding issue.

A divergence with a BLOCKING reason (`lookup-failed`, `not-seen`) must be investigated before the deployment is accepted. A divergence with only legitimate reasons (`no-dep-time`, `past-time`, `beyond-window`) is a PASS WITH NOTE — the tier behaved correctly on the available data.

**Divergence-report rejection semantics — `out-of-radius` classification refinement (v5.9.7 CC4 — MANDATORY):** The `out-of-radius` reason is BLOCKING ONLY when no other cascade tier matched the same trip in the same cycle. If T1 exact-stop-id, T2 alt-stop-id, or T3 all-trips-scan already matched the trip at a higher-confidence tier (observable via `tramMatchTier !== 'coord-identity'`), the tier-4 `out-of-radius` signal is a data-quality observation about the static dataset's paired-platform coordinate registration — not a cascade regression. In that case the correct classification is **PASS WITH NOTE**. The `TRAM_COORD_IDENTITY_RADIUS_METRES = 40 m` constant is correct for the bulk of the Melbourne tram network per the measured 23.3 m paired-platform distance documented in v5.9.3 Y1 and **MUST NOT** be widened in response to individual coordinate-registration outliers — widening the radius beyond 40 m would risk false matches against neighbouring physical platforms. A future audit cycle may address specific outliers in `VIC_TRAM_STOPS_WITH_COORDS` via a GTFS re-import and independent cross-reference against the Transport Victoria static source, but v5.9.7 documents the known limitation without modifying the static dataset. v5.9.6 swarm Agent A observed an entry with `nearestStu.distance` ≈ 285 m (much larger than the paired-platform distance) while T2 alt-stop-id had already matched the same feed stop at the same cycle — the contextual classification that prevented a false BLOCKING verdict is now formalised in this rule.

Historical context: the v5.9.4 rule said "if stop-level succeeded but `matchedCount === 0`, that is a regression". That wording was too coarse — it flagged stu-level edge cases as regressions when they were legitimate. The v5.9.5 AA1 fix (moving depTime validation into the inner loop) eliminates the "nearest stu invalid blocks trip" class of bug, and the v5.9.5 AA3 telemetry + this rule surface any remaining divergence with precise rejection reasons. The v5.9.7 CC4 refinement distinguishes cascade-health regressions (BLOCKING) from data-quality observations about the static dataset (PASS WITH NOTE).

**Stability requirement (v5.9.3 Y6, revised in v5.9.5 AA4 — MANDATORY):** Agent A MUST fetch `/api/commutecompute?format=json` twice with a 35-second interval (GTFS-RT cache TTL 30 s + 5 s buffer per §11.1). The stability invariant is three conditions that MUST hold simultaneously:

1. **Identical displayed route identifier** across both fetches — if the rendered `tramRoute` number differs, that is a BLOCKING regression (indicates KV oscillation or cascade instability).
2. **`preferredTramRoute` KV value is non-null** after the first post-deploy cycle completes — if the stored preference remains null, either the Z2 gate rejected every stop-level opportunity OR the Z3 migration wiped without re-population; either case is a BLOCKING issue requiring investigation.
3. **Across a longer sliding window (8-fetch run over ~5 minutes) at least one fetch SHOULD land on an identity tier** (`exact-stop-id`, `lenient-scan`, or `coord-identity` confidence). This proves the feed does periodically contain the target stop in some trip's stu sequence. If NONE of 8+ fetches land on identity tier AND the feed has non-zero entities, the stop may be in a chronically feed-sparse state — ADVISORY warning, not BLOCKING. Route-level fallback via KV is acceptable indefinitely as long as conditions 1 and 2 hold.

The v5.9.3 Y6 rule wording ("both responses MUST have identity tier") was too strict for the GTFS-RT sparsity reality and has been corrected in v5.9.5 AA4.

**Cold-start assertion (v5.9.4 — MANDATORY):** Agent A MUST inspect `_liveDataDiag.tramSelectionBranch` on the first post-deploy request. If the branch is `frequency-detected-coord` or `feed-majority` AND the stored `cc:preferred_tram_route` KV value is null (cold start), the Z2 gate must have prevented the KV write. If the gate let a non-stop-level winner into KV, that is a cold-start poisoning regression.

**Cross-reference requirement:** After every tram pipeline change, Agent A MUST cross-reference the selected tram route against Transport Victoria's `/departures` page for the same stop. A mismatch is a BLOCKING regression.

**Tier consistency invariant (v5.9.5 AA1 — MANDATORY):** `processCoordIdentityMatch` and `processGtfsRtDepartures` MUST use identical stu-level validation semantics: both functions validate `depTime` presence, past-departure (`mins > 0`), and forward-window (`mins ≤ 120`) PER stu inside the inner loop, before any cross-stu selection logic. The coord-identity tier's "nearest within radius wins" comparison MUST operate only on stus that have already passed the per-stu validation. This rule was added after the v5.9.4 live verification exposed a case where coord-identity picked a `matchedStu` on distance alone and then bailed on the whole trip if `matchedStu` had invalid depTime, silently losing other valid stus within the same trip. Regression tests in `tests/test-tram-cascade-selection.js` cover this invariant.

**Station override cascade invariant (v5.9.6 BB1 — MANDATORY):** When a station override is applied via `cc:station_overrides` (POST /api/routes with `{stationOverrides: {...}}`), the engine MUST treat the override as a first-class input to downstream walk-leg and interchange computation — NOT as a late-stage label-only patch. Specifically:

1. The override's representative coordinates MUST be resolved via `lookupMetroStationCoords(id)` (in `src/data/gtfs-stop-names.js`) and cascaded through every leg that borders the overridden transit mode.
2. Walk legs that terminate at the override'd train station MUST have their duration recomputed using `haversine` from the effective walk origin (the tram alighting stop, for tram→train interchange legs) to the override's coordinates. The walking-speed constant is `80 m/min` per §23.13 allowlist.
3. Walk leg titles MUST display the override's station name (from `overrideTrainName = trainOverride.name || getStopNameById(trainStopId)`), not the engine's auto-detected nearest-station name.
4. The tram alighting stop MUST be re-selected via `findNearestTramStopNearCoords(overrideLat, overrideLon)` when a train-side override is active. The boarding stop (nearest tram stop to HOME) stays unchanged; only the alighting stop re-derives. Route membership is not enforced in the helper because `VIC_TRAM_STOPS_WITH_COORDS` does not store per-stop route metadata — the downstream GTFS-RT cascade re-verifies route membership at runtime.
5. Walk legs following a train leg (where the user walks FROM the train station to a destination) MUST recompute duration from the override coordinates to the leg's stored destination coordinates.

Violation pattern: patching `leg.stationName` on the leg object without updating the adjacent walk-leg durations or the tram alighting stop — this produces correct titles but wrong durations and wrong route geometry. The v5.9.5 deploy exhibited this violation during the station-override live test; v5.9.6 BB1 closed it by extending the existing post-engine override block in `api/commutecompute.js` to perform the full coordinate cascade. Regression tests in `tests/test-tram-cascade-selection.js` cover the `lookupMetroStationCoords` and `findNearestTramStopNearCoords` helpers.

**Train cascade stop-level gate (v5.9.6 BB3 — MANDATORY, symmetric with v5.9.4 Z2):** The train cascade in `api/commutecompute.js` (around line 3385-3430 after the v5.9.6 BB3 edit) MUST apply the same stop-level confidence gate as the v5.9.4 Z2 tram gate. A `setPreferredTrainLine` / `setPreferredTrainStation` KV write is permitted only when BOTH conditions hold:

1. `preferredTrainLine` is currently empty (write-once constraint), AND
2. The dominant live line has at least one departure with a `source` value in `TRAIN_STOP_LEVEL_SOURCES = { 'gtfs-rt', 'gtfs-rt-scan', 'gtfs-rt-coord-identity' }`.

**`gtfs-rt-broad` is EXCLUDED** from the train stop-level set. The broad-fallback tier (`processAnyRouteDepartures` in `src/services/opendata-client.js`) is a mode-wide "any train in the feed" fallback used when the exact stop has no trip updates — it is NOT a stop-level match. A broad-fallback dominant line MAY drive the current request's displayed line but MUST NOT persist to KV. Likewise `gtfs-rt-route` (route-level median-stop heuristic) is excluded for the same reason.

Historical context: v5.9.4 Z2 added this gate for trams only. v5.9.6 BB3 extends it to trains symmetrically, closing the "train KV can be poisoned by broad-fallback on cold-start" class of bug that mirrors the v5.9.3 tram cold-start poisoning. The v5.9.6 BB4 migration wipe (`cc:v596_migration_done`) clears any suspect value already persisted by the pre-gate v5.9.0-through-v5.9.5 train cascade — symmetric with v5.9.4 Z3 but for train keys.

**`nextDepartures` truthfulness invariant (v5.9.6 BB2 — MANDATORY):** The `nextDepartures` catchable list on a journey leg MUST contain ONLY departures derived from the raw GTFS-RT feed (via `processGtfsRtDepartures`, `processAllTripsStopSearch`, `processRouteLevelDepartures`, `processCoordIdentityMatch`, or the broad fallback). Headway-based projection into the future is **PROHIBITED** — if the live feed has fewer than N catchable departures after the user's arrival at the stop, the subtitle MUST show fewer entries, NOT fabricate additional ones via headway extrapolation.

The subtitle " LIVE" suffix applies only when `baseLeg.isLive` is true AND all entries displayed come from the live feed. After v5.9.6 BB2 removed the two headway-extrapolation paths in `api/commutecompute.js` (the pre-projection block around line 735 and the tier-2 padding block around line 1068), the `isLive` flag is truthful by construction because the list is populated only from real feed entries.

If after the projection-removal a leg has zero catchable departures, the existing timetable-fallback path (`isTimetableEstimate: true` → subtitle "Scheduled ~Xmin") correctly handles the no-live-data case per §23.6. v5.9.6 BB2 does not change the timetable-fallback path; it only closes the pre-v5.9.6 loophole that let projected entries bypass it and masquerade as LIVE.

**Catchability filter invariant (v5.9.7 CC2 — MANDATORY):** The `nextDepartures` array stored on a journey leg MUST contain only departures the user can physically catch given their cumulative walk time and the standard two-minute catchability buffer (i.e. every retained entry satisfies `departureMs >= arrivalAtStopMs - 2*60000`). Pre-v5.9.7 code padded the array from a "from now" forward-window list (filtered only by `depMs > nowMs`, without applying the arrival-at-stop catchability buffer) whenever fewer than three catchable entries existed, producing an inconsistency between the subtitle (catchable-filtered by the subtitle constructor) and the raw `leg.nextDepartures` array (unfiltered). Downstream consumers reading the raw array directly — the renderer, firmware, telemetry, the admin diagnostics panel, and the `/cc-deploy-verify` Agent C cross-reference — saw uncatchable entries the user could never board. The v5.9.7 CC2 fix removes the padding loop: if fewer than three catchable entries exist, fewer are shown; the list is never extended with uncatchable entries to "pad to three". An additive `allNextDeparturesMs` telemetry field provides the full forward-window list (sorted) for diagnostic consumers that need visibility into the raw feed, but downstream code that renders to the user MUST read `nextDepartures`, not `allNextDeparturesMs`. The same catchability filter MUST be applied to every code path that writes to `leg.nextDepartures` — including the secondary supplement loop that pads from the raw feed — so no path can reintroduce uncatchable entries. Violation pattern: any code path that pushes minute values into `leg.nextDepartures` without first re-applying the `arrivalAtStopMs - catchabilityBuffer` filter.

**Train leg timestamp lossless-reconstruction invariant (v5.9.7 CC1 — MANDATORY):** When a live feed entry is matched to a train leg and the rendered fallback path `liveData.minutes !== undefined` is taken, the derived `departMs` MUST prefer `liveData.departureTimeMs` verbatim when that field is present, and ONLY fall back to `nowMs + (liveData.minutes * 60000)` when the raw timestamp is genuinely absent. The pre-v5.9.7 reconstruction unconditionally rebuilt `departMs = nowMs + (liveData.minutes * 60000)`, which silently round-tripped the real timestamp through a rounded-to-the-nearest-minute integer (because `liveData.minutes` was itself computed via `Math.round((depMs - nowMs) / 60000)` earlier in the pipeline). On the common live path the raw value was always available, so the lossy reconstruction overwrote it with up to ±30 s of drift — a non-user-visible but telemetry-visible artefact that showed up as a ~24 s gap between the train leg's `nextDepartureTimesMs[0]` and the raw feed's `departureTimeMs` for the same trip. v5.9.7 CC1 restores lossless reconstruction. Violation pattern: any new code path that recomputes `departMs` from `nowMs + minutes * 60000` without first checking for a raw timestamp.

Historical context: v5.9.5 live verification showed a train leg with subtitle "Next: 24, 44, 64 min LIVE" where only the 24-min entry was a real GTFS-RT match — 44 and 64 were computed from the observed headway between the first two real entries (~20 min) and added to both `nextDepartures` and `nextDepartureTimesMs` without any source metadata. The subtitle's "LIVE" suffix was applied leg-level not entry-level, so the user saw three "LIVE" values where only one was genuinely live. v5.9.6 BB2 removes the extrapolation entirely; the parallel telemetry field `nextDeparturesSources[]` is deferred to a future cycle because removing the projection makes the `isLive` flag truthful on its own.

**Rationale:** Every tram-route-identification bug in the v5.8.1 → v5.9.5 series was a symptom of one of these architectural flaws: (1) the cascade treated coord-proximity (300 m) as a primary identity mechanism rather than a last-resort fallback — fixed in v5.9.2 cascade reorder; (2) the identity tier was silently neutered by a too-tight radius — fixed in v5.9.3 Y1; (3) a route pre-filter — fixed in v5.9.4 Z1; (4) naive KV write semantics that poisoned the stored preference on cold start — fixed in v5.9.4 Z2 + Z3; (5) the nearest-wins inner loop picking a `matchedStu` on distance alone and bailing on the trip when that stu's depTime was invalid — fixed in v5.9.5 AA1; (6) stop-level cascade accounting that conflated T1 and T2 counts — fixed in v5.9.5 AA2; (7) rules that demanded always-identity-tier confidence and didn't acknowledge GTFS-RT feed sparsity — corrected in v5.9.5 AA4. The v5.9.6 corrective adds four additional invariants surfaced by the station-override live test: (8) station overrides must cascade through walk-leg coordinates, not just labels (BB1); (9) `nextDepartures` must not be extrapolated via headway projection (BB2); (10) the train cascade must apply the same stop-level gate as the tram cascade (BB3); and (11) a one-off migration wipe must clear any pre-gate train KV values (BB4). After v5.9.6, the full transit identification and rendering pipeline is: identity-first cascade order (§23.15), named constant radius (§13.3), no pre-filter on identity (Z1 rule), stop-level-gated KV writes for BOTH tram and train (Z2 + BB3 rules), one-off poisoning wipes (Z3 + BB4 rules), full diagnostic telemetry (Y4 + Z4 rules), cross-tier divergence telemetry (AA3 rule), per-stu depTime validation in both tiers (AA1 rule), separate T1/T2 accounting (AA2 rule), feed-sparsity-aware stability verification (AA4 rule), station override coordinate cascade (BB1 rule), truthful `nextDepartures` list (BB2 rule), and regression tests covering every new invariant (`tests/test-tram-cascade-selection.js`).

---

### 23.16 Home Coordinate Freshness Invariant (v5.9.8)

**[CRITICAL]:** Every downstream consumer of the stored home coordinate — the nearest-stop finder, the tram cascade target-coord resolver, the dynamic-route engine's walk-leg computation, and every diagnostic helper that exposes `homeCoords` — MUST treat `kvPrefs.locations.home.lat/lon` as conditionally trusted. Trust is gated by a short hash of the canonicalised `home.address` that is persisted alongside the coord.

**Background:** Pre-v5.9.8 code paths wrote `locations.home.lat/lon` blindly from whatever the Setup Wizard, admin panel, or address auto-complete last supplied. Any drift between the stored coord and what a fresh geocode of the stored address would resolve — from Setup Wizard write vs. later admin panel edit, geocoder vendor drift between configuration and read, manual pin drag, a geocoder-tier fallback (Google Places → Nominatim) producing a slightly different lat/lon, or a stale coord surviving an address change — poisoned every downstream consumer simultaneously. The nearest-stop finder picked the wrong boarding station because its input was off by 100+ m; the tram cascade's coord-identity tier (40 m radius per §13.3) compared feed stops against a target that was outside the radius of the correct boarding stop; walk-leg distances were computed from the wrong origin. A single corrupt coord produced a fully-consistent-but-wrong journey response, with no error path because every downstream consumer silently trusted the input.

**Required invariant (v5.9.8 DD1 — MANDATORY):** At the request boundary in `api/commutecompute.js::getEngine`, before building the engine-level `preferences` object, the stored home record MUST be passed through `ensureFreshHomeCoords(kvPrefs, diag)`:

1. If `locations.home.address` is absent, short-circuit with source `'no-address'` — nothing to verify against.
2. If `locations.home._addressHash` equals the short hash of the canonicalised address AND a coord is present, trust the stored coord (fast-path source `'cached-hash-match'`).
3. Otherwise, call the project's geocoding service (`src/services/geocoding-service.js::GeocodingService.geocode`) with a short timeout (`HOME_COORD_GEOCODE_TIMEOUT_MS = 5000`). On failure, return the stored coord with source `'geocode-failed'` — the request MUST NOT abort on a geocoding failure.
4. Compare the fresh geocode against the stored coord via haversine:
   - If the drift exceeds `HOME_COORD_REJECT_THRESHOLD_M = 2000`, treat the fresh result as a vendor misresolve and keep the stored coord (source `'geocode-rejected-too-far'`).
   - If the drift is at or below `HOME_COORD_DRIFT_THRESHOLD_M = 75`, keep the stored coord (vendor jitter) and only refresh the hash (source `'cached-drift-ok'`).
   - Otherwise, overwrite `locations.home.lat/lon` with the fresh values and write the updated preferences back to KV (source `'geocode-drift-corrected'`).
5. The per-serverless-instance memory cache keyed by the canonicalised address avoids hitting the free-tier geocoder on repeat requests within a warm-window (`_freshHomeCoordMemoryTtlMs = 10 min`).

**Required migration (v5.9.8 DD4 — MANDATORY):** A one-off KV flag `cc:v598_home_coord_migration_done` MUST be used to ensure the DD1 drift check runs at least once for every user after v5.9.8 deploys. The DD1 implementation satisfies this automatically because pre-v5.9.8 KV entries have no `_addressHash` field, forcing the drift check on first post-deploy request. The flag is reserved for future repair cycles that may need a second pass. Symmetric with the v5.9.4 Z3 and v5.9.6 BB4 migration patterns.

**Required telemetry (v5.9.8 DD1 — MANDATORY):** `_liveDataDiag.homeCoordFreshness` MUST be present on every response that reached engine initialisation. Possible `source` values: `'cached-hash-match'`, `'memory-cache'`, `'cached-drift-ok'`, `'geocode-first'`, `'geocode-drift-corrected'`, `'geocode-rejected-too-far'`, `'geocode-empty'`, `'geocode-failed'`, `'no-address'`, `'not-run'`. The field is the primary evidence a post-deploy verification pass reads to confirm DD1 executed; a missing field or `source === 'not-run'` indicates regression.

**Tram stop coord override hardening (v5.9.8 DD2 — MANDATORY, companion to §23.15):** The tram stop coord override block in `api/commutecompute.js` that replaces `tramApiOptions.lat/lon` with the locked tram stop's dataset coord MUST emit `_liveDataDiag.tramStopCoordOverride` with `{ attempted, tramStopId, resolved, fallbackUsed, resolvedLat, resolvedLon }`. When the exact-id dataset lookup fails, the block MUST NOT silently leave `tramApiOptions.lat/lon` at the (possibly drift-corrected) home coord; instead it MUST attempt `findNearestTramStopNearCoords` around the home coord and use the nearest tram stop's coord as the cascade target (`fallbackUsed: 'nearest-stop-helper'`). Only if that also fails may it preserve the home coord as a last resort (`fallbackUsed: 'home-coords-last-resort'`), and the telemetry MUST explicitly reflect that — `fallbackUsed === null` means "we tried and succeeded", anything else means "diagnose why".

**Violation patterns:**
- Any new code path that reads `locations.home.lat/lon` or `cache.homeCoords` without first going through `ensureFreshHomeCoords` at the request boundary. The boundary check is the ONLY authorised entry point.
- Any new tram cascade call site that sets `tramApiOptions.lat/lon` from home coords without subsequently overriding via the DD2 hardened block.
- Any diagnostic field that exposes `homeCoords` or nearest-stop candidates without ALSO exposing `homeCoordFreshness` — observability without the freshness verdict is a debugging trap.

**Regression tests:** `tests/test-tram-cascade-selection.js` contains pure-function replicas of both `ensureFreshHomeCoords` (seven tests covering the fast path, drift correction, sanity guards, timeout fallback, and empty-result fallback) and the DD2 tram stop coord override hardening (five tests covering success, nearest-helper fallback, last-resort fallback, no-home-coords, and no-tram-stop-id cases). When the inline code in `api/commutecompute.js` changes, the replicas MUST be updated in lock-step.

**Serverless geocoding limitation and direct-entry escape hatch (v5.9.9 EE1 — MANDATORY):** The v5.9.8 DD1 home coord freshness invariant relies on runtime geocoding of `locations.home.address` via `src/services/geocoding-service.js::GeocodingService`. On the Vercel serverless production environment, all five geocoder tiers (Google Places, Mapbox, HERE, LocationIQ, Nominatim) have been observed to fail simultaneously: the commercial tiers because no API keys are configured in the Zero-Config environment (§24.1 self-hosted distribution model), and Nominatim either because of IP-range rate-limiting or blocking applied to the serverless provider's outbound IPs, because the 5 s serverless cold-start timeout is tighter than the Nominatim handshake window, or because the User-Agent triggers an abuse filter. DD1 correctly falls back to the stored coord on failure (the request never aborts — that part of DD1 is sound), but a stored coord that was itself corrupt cannot be repaired by a geocoder that is non-operative.

v5.9.9 EE1 closes this gap by adding a direct-entry escape hatch to `/api/admin/preferences` POST: an optional `lat`/`lon` pair in the request body is accepted verbatim, skipping the geocoder, and written with the v5.9.8 `_addressHash` field so the DD1 fast path triggers immediately on subsequent reads — no runtime geocoder dependency at all. The admin panel UI (`public/admin.html`) exposes optional text inputs for lat/lon in a collapsible "Optional: paste lat/lon to bypass geocoding" disclosure alongside every address field, so users can paste coordinates from any mapping service (Google Maps right-click → copy, Apple Maps share → copy, OpenStreetMap URL fragment) to repair a corrupt stored coord. When a caller supplies a valid lat/lon pair, the EE1 handler also pre-computes the same SHA1-based address hash that v5.9.8 DD1 expects, so the admin panel + cascade work together without any runtime geocoding round-trip.

**DD1 geocoder-failure memoisation (v5.9.9 EE3 — MANDATORY):** The v5.9.8 DD1 helper MUST cache geocoder failures for a short TTL so that a known-broken serverless environment does not thrash the geocoder endpoint on every request. The v5.9.9 EE3 hardening stores `{ failed: true, reason, timestamp }` in `_freshHomeCoordMemoryCache` keyed by the canonicalised address when the geocoder throws OR returns an empty result, with a 60 s TTL (shorter than the 10 min TTL used for success entries so a transiently-broken environment can recover quickly). Subsequent requests within the TTL short-circuit to the `memory-cache-failed` source verdict and return the stored coord without calling the geocoder. A `_liveDataDiag.homeCoordFreshnessFailureCount` counter surfaces how many NEW failures were recorded by the current serverless instance, distinct from how many requests the cache served. `memory-cache-failed` is an expected and healthy verdict on a broken environment — it indicates the cache is actively protecting the free OSM Nominatim infrastructure and the user from per-request latency penalties. It is NOT a regression and does NOT require a new cycle to investigate unless the failure count climbs without bound.

**Violation patterns:**
- Any new serverless code path that relies on runtime geocoding without a direct-entry fallback. Runtime geocoding MUST be treated as best-effort, never as the sole repair mechanism for a corrupt stored coord.
- Any admin or Setup Wizard flow that writes `locations.*.lat/lon` without also writing the `_addressHash` — this breaks the DD1 fast path and forces a geocoder call on the very next read.
- Any future change to `_addressHash` in `api/commutecompute.js` that is not mirrored into `_addressHashForCanonical` in `api/admin/preferences.js` — the two implementations MUST produce identical hashes for the same canonicalised address or the write/read round-trip silently misses the fast path.
- Any diagnostic tooling that classifies `source: 'memory-cache-failed'` as a blocking regression without first checking the failure count trajectory and the environment's geocoder availability.

**Regression tests (EE4):** `tests/test-tram-cascade-selection.js` adds a pure-function replica of the EE1 direct-entry branch (five tests covering valid direct coord bypass, missing lat fallback, invalid numeric fallback, out-of-range fallback, and the round-trip where EE1's written hash triggers DD1's fast path on the subsequent read). Same lock-step update requirement as the other replicas in the file.
